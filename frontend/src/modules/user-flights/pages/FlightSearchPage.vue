  ...
  <section class="flight-search-page">
    ...
    <section data-testid="dashboard-ai" class="flight-search-page__ai-section">
      <div class="flight-search-page__card-head">
        <div>
          <h2>{{ aiText.title }}</h2>
          <p class="flight-search-page__section-note">{{ aiText.description }}</p>
        </div>
      </div>
...
defineOptions({ name: 'FlightSearchPage' })
...
function saveSearchState() {
  ...
  const favEntries = []
  favoriteStatusMap.value.forEach((v, k) => favEntries.push([k, v]))
  sessionStorage.setItem(PERSIST_KEY, JSON.stringify({
    filters: { ...filters },
    flights: flights.value,
    selectedFlightId: selectedFlightId.value,
    currentPage: currentPage.value,
    syncResult: syncResult.value,
    syncForm: { ...syncForm },
    syncDetailsOpen: syncDetailsOpen.value,
    favoriteStatusMap: favEntries
  }))
}
...
function restoreSearchState() {
  try {
    ...
    if (!state.flights?.length) return false

    Object.assign(filters, state.filters || {})
    flights.value = state.flights
    currentPage.value = state.currentPage || 1
    syncResult.value = state.syncResult || null
    syncDetailsOpen.value = state.syncDetailsOpen || false
    Object.assign(syncForm, state.syncForm || { airportCode: 'CKG', date: getTodayDateString() })

    if (state.favoriteStatusMap?.length) {
      const map = new Map()
      state.favoriteStatusMap.forEach(([k, v]) => map.set(Number(k), v))
      favoriteStatusMap.value = map
    }

    if (state.selectedFlightId) {
      const found = flights.value.find(f => f.id === state.selectedFlightId)
      if (found) selectedFlight.value = normalizeFlightForDisplay(found, locale.value)
...
.flight-search-page__ai-section {
  display: grid;
  gap: 10px;
  padding: 16px;
  background: #ffffff;
